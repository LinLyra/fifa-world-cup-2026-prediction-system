...
OUT_FILE = OUT_DIR / "poisson_group_predictions_v1.csv"
...
def load_strength_tables():
    ...
    df = ad.merge(elo.rename(columns={"elo": "custom_elo"}), on="team", how="left")
...
def score_matrix(lambda_a, lambda_b, max_goals=MAX_GOALS):
    ...
    for hg in range(max_goals + 1):
        for ag in range(max_goals + 1):
            p = poisson.pmf(hg, lambda_a) * poisson.pmf(ag, lambda_b)
            matrix[(hg, ag)] = p
